...
#[tauri::command]
pub async fn send_local_input(session_id: String, data: String) -> Result<Value, String> {
    Box::pin(async move {
        let state = crate::state::require_state()?;
...
#[tauri::command]
pub async fn close_local_terminal(session_id: String) -> Result<Value, String> {
    Box::pin(async move {
        let state = crate::state::require_state()?;
        let mut map = state.local_terminals.lock().map_err(|e| e.to_string())?;
