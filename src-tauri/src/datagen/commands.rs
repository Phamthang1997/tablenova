...
use super::meta::{FkMeta, collect_meta, topo_order};
use super::spec::{Cell, GenSpec, o_str};
...
pub async fn get_generation_targets(conn_id: String) -> Result<Value, String> {
    Box::pin(async move {
        ...
        let fk_map: HashMap<String, Vec<FkMeta>> = metas
            .iter()
            .map(|m| (m.name.clone(), m.fks.clone()))
            .collect();
        let (order, cyclic) = topo_order(&names, &fk_map);

        let mut tables_json = Vec::with_capacity(metas.len());
        for name in &order {
            let Some(meta) = metas.iter().find(|m| &m.name == name) else {
                continue;
            };
...
#[tauri::command]
pub async fn cancel_data_generation(conn_id: String) -> Result<Value, String> {
    Box::pin(async move {
        let state = crate::state::require_state()?;
    ...
    Box::pin(async move {
        ...
        match outcome {
            Ok((inserted, cancelled)) => {
                ...
                let inserted_json: Map<String, Value> = inserted
                    .iter()
