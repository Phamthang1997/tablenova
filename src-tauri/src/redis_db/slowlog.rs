...
pub(crate) fn parse_slowlog_entry(v: &redis::Value) -> Option<Value> {
    let redis::Value::Array(a) = v else {
        return None;
    };
    if a.len() < 4 {
        return None;
...
#[tauri::command]
pub async fn redis_slowlog_get(conn_id: String, count: Option<usize>) -> Result<Value, String> {
    Box::pin(async move {
...
pub async fn redis_slowlog_reset(conn_id: String) -> Result<Value, String> {
    Box::pin(async move {
        ...
        let mut c = take_conn(&state, &conn_id)?;
        let _: String = redis::cmd("SLOWLOG")
            .arg("RESET")
            .query_async(&mut c)
            .await
            .map_err(|e| e.to_string())?;
